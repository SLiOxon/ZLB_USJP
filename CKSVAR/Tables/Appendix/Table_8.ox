//	Tests ZLB irrelevance hypothesis in US and Japan by testing CSVAR against CKSVAR, with credit spread
...
main()
{
	...
	for(decl irate = 0; irate < 2; ++irate){
	...
	decl p = iJP ? 2 : (irate ? 3 : 4);						// lag length determined by the aic, if US and BAA10Y, then p = 3
	...
	//	Setup the model	
		...
		model.Select(CKSVAR::Y1_VAR, {"INFL", 0, p, iJP ? "YGAP_BOJ" : "YGAP", 0, p, iJP? "LR9Y" : "LONGRATE", 0, p, irate ? "BAA10Y" : "EBP", 0, p});	  // Select Y1 (unconstrained) variables in the SVAR, for US, can also replace with EBP with BAA10Y
		...
		decl iYear0 = iJP ? 1985 : irate ? 1987 : 1974;
		decl iQuarter0 = iJP ? 3 : 2;
