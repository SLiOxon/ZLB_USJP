/**	Tests exclusion of lags of longrate in US and Japan
	The full model is CKSVAR
	Note that you may need to experiment with fUseKSVARinit to get estimation with higher likelihood
**/
...
main()
{
	...
	for(decl iJP = 1; iJP < 2; ++iJP){			// do US and JP
	...
	for(decl ilr = 1; ilr < (iJP? slrs_jp : slrs_us); ++ilr){
	...
	for(decl ip = 0; ip < pmax-pmin+1; ++ip){									// VAR order
		...
		decl sestimfile = sprint(iJP ? "JP_" : "US_", "p_",p,"_R_",R,"_",iJP? lrs_jp[ilr] : lrs_us[ilr]);
	///////////////////////////////////////////////////////////////////////
	//	Setup the model	
		...
		ranseed(-1);
		decl vp0;
		if(iJP && (ilr == 0 ||ilr == 2 || ilr == 4 || ilr == 5)){
		model.Estimate();
		vp0 = model.GetPar();
		}
		...
		if(p){
			...
			decl asY1lags;
			model.GetGroupLagNames(CKSVAR::Y1_VAR, 1, p, &asY1lags);
			decl asY1lags2excl = asY1lags[(cY1-1)*p:];
			for(decl j = 0; j < sizeof(asY1lags2excl); ++j)
				vIdxExcludeLags |= find(model.GetParNames(), sprint("Eq.", cY1+1, " ", asY1lags2excl[j]));
	...
	//  Impose exclusion restrictions and estimate
		...
		if(p){
			...
			for(decl i = 0; i < cY1-1; ++i)
				for(decl j = 0; j < sizeof(asY1lags2excl); ++j)
					vIdxExcludeLags |= find(model.GetParNames(), sprint("Eq.", i+1, " ", asY1lags2excl[j]));	//only exclude from eq. 1 and 2
			for(decl j = 0; j < sizeof(asY1lags2excl); ++j)
				vIdxExcludeLags |= find(model.GetParNames(), sprint("Eq.", cY1+1, " ", asY1lags2excl[j]));
			model.FixPar(vIdxExcludeLags, zeros(sizerc(vIdxExcludeLags), 1));
		}
