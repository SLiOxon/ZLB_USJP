...
model; 
  ...
  # PLU = 1/(RbLU-kap_p);
  %# tauU = (1+kap_p*PLU)*bLU(-1)/(piU)+(1+iU(-1))*bU(-1)/(piU) - (bU + PLU*bLU);
  # profU = omegau_p*zetaU*PLU*bLuU + yU-wU*yU^(1/alpha_p);
  %crU + PLU*bLrU = PLU*RbLU/(piU)*bLrU(-1) + wU*hrU + profU - tauU;
  ...
  log(bLU/steady_state(bLU)) = gam_p/(1+iSS)*(iTU-alphaZ_p*ZU);
  %# gam_p0 = ((1-omegau_p)*steady_state(crU)/steady_state(yU)*zetab_p/(1+zetab_p)*rhozeta_p)^(-1);
  %log(bLU/steady_state(bLU)) = gam_p0*(iTU-alphaZ_p*ZU);  
  ...
  @# endif
  @# if ms == 1
      # rhozeta_p0 =  ((1-omegau_p)*steady_state(crU)/steady_state(yU)*zetab_p/(1+zetab_p)*gam_p)^(-1);
  @# endif
  @# if ms == 2
      # rhozeta_p0 = eff_p*((1-omegau_p)*steady_state(crU)/steady_state(yU)*zetab_p/(1+zetab_p)*gam_p)^(-1);
  @#endif
  @# if ms == 3
      # rhozeta_p0 = rhozeta_p;
  @#endif
  zetaU = zetab_p*(bLU/steady_state(bLU))^(rhozeta_p0);
