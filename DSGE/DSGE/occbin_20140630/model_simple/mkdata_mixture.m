function [zdata]=mkdata_mixture(nperiods,decrulea,decruleb,decrulea_star,decruleb_star,endog_,exog_,wishlist,irfshock,scalefactormod,violvecbool,steady_state,steady_state_star,init)
...
for i = 2:nperiods+1
    ...
    if i<=(lengthshock+1)
        ...
        history(:,i) = decrulea_star * (history(:,i-1)+steady_state-steady_state_star)+decruleb_star*errvec ...
                       + steady_state_star - steady_state;
    else
    % update endogenous variables
    history(:,i) = decrulea_star * (history(:,i-1)+steady_state - steady_state_star) + steady_state_star - steady_state;
    end
    end
end
