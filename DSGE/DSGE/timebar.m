...
if nargin == 0
    message = [evalin('caller', 'mfilename') '@' datestr(now)];
end
if ~ishandle(message)                                       % If first input is not a timebar handle, 
    ...
    est_text = 'Remaining time (est.): ';                   % Set static estimated time text
    ...
    userdata.text(4) = uicontrol(h,'style','text',...       % Prepare the percentage progress
        'pos',[winwidth-35 winheight-50 35 20],...          % Set the textbox position and size
    ...
    userdata.time = [];                                     % On opening don't set time
    userdata.inc = [];                                      % 
    ...
    if isempty(userdata.time)
        userdata.time = clock;                                % Initialize the current time
...
time_message = sprintf('%02d:%02d''%.1f', hours,minutes,seconds);
time_message = sprintf('%s (%02d:%02d''%.1f)', time_message, tpast(3)*24+tpast(4),tpast(5), tpast(6));
...
%             time_message = strcat(h0,num2str(hours),'h',m0,...
%                 num2str(minutes),'''',s0,num2str(seconds),...
%                 '''''',num2str(tenths),...
%             '(',num2str(tpast(3)*24+tpast(4)),'h',...
%             num2str(tpast(5)),'''',...
%             num2str(tpast(6)),'''''');       % Format estimated time as hh:mm:ss.t
        ...
        set(userdata.text(4),'string',...
         ...
         set(h, 'Name', ['Progress... ' get(userdata.text(4),'string')])
    end
