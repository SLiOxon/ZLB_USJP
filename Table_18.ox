/**	Tests attenuation bias with alternative sample periods
	The full model is CKSVAR
	May need to experiment with fUseKSVARinit to see which achieves higher likelihood
**/
...
main()
{
	...
	for(decl idate = 0; idate < 2; ++idate){			// do US and JP

	decl fUseKSVARinit = iJP ? FALSE : FALSE;	// if TRUE, estimate KSVAR first and use as initial values for estimation of CKSVAR. KSVAR initialisation performs better than without it for US data but worse for Japan

	decl pmax = iJP ? 5 : 5;
	decl pmin = 1;
	decl b = iJP ? 0.07 : 0.2;					// lower bound = IOR+0.07 for JP, 0.2 for US
	decl mresults = constant(.NaN, pmax-pmin+1,8);	// store p, lik, cp, pv-p, aic
	...
	//	Setup the model	
		...
		model.SetSelSample(/*Start Year*/iYear0,/*Start Period*/ iQuarter0, /*End Year*/ iYear1, /*End Period*/ iQuarter1);		// -1 defaults to earliest (for start) or latest (for end) available dates in the sample 
...
//		}
		...
		decl asnames = {"p", "lik", "par","pv-p", "aic"};
