...
**/
{
	...
	m_cPar += m_cBreakPar;
	m_mxi = ((sizeof(m_mxi) != m_cT) || (sizec(m_mxi) != m_cR)) ? ranu(m_cT, m_cR) : m_mxi;		// draw uniform random numbers to integrate out Y2* from the likelihood using importance sampling (or do nothing if already drawn earlier)
...
//			varg2gb[idxr] = (m_mY2[idxr] - vmu2[idxr])/(i ? atau[i-1] : tau);			// kernel of contribution of Y2 to the likelihood when Y2>b	 (gb stands for greater than b)
			...
			// now move to case Y2 = b
			decl vmu2i = vmu2[idxr];
			if(m_cY1)
				vmu2i += (m_mY1-mmu1)[idxr][]*aarg2[i]';				// mean of Y_2* given Y1 when Y2* < b
...
**/
{
	...
	decl mR = m_cY1 ? unit(m_cPar)[m_vIdxbetatilde][] : <>;	 // restrictions of the form mR*b=0
	decl vr = m_cY1 ? zeros(sizer(mR),1) : <>;
	if(m_fLatLags){
		decl vIdxLatLags = m_fdiffY2 ? dropr(m_vIdxLatLags, m_cY2lags)	// if contrained variable is in first difference, drop the coef in the lagged latent level in the Y2 eqn, because it is not constrained to be zero
					 : m_vIdxLatLags;									// otherwise, use them all
		...
		if(m_fdiffY2){
			mR |= unit(m_cPar)[m_vIdxLatLags[m_cY2lags]][];				// m_fdiffY2 == TRUE, also impose restriction that coef on lagged latent level = 1.
			vr |= 1;
		}
	}
	decl fPrint = m_fPrint;
...
**/
{
	...
	if(m_cY1){
		...
		vgamma = vbetabar = 0;
		mOmega = tau^2;							// set to zero if empty, to avoid conditional statements where these appear below
	}
	...
	for(decl j = 0; j < m_iSol; ++j){
		...
		mOm11bar_ch = m_cY1 ? choleski(mOm11bar) : 0;
		...
		decl u2 = (m_dImpulse + (m_cY1 ? meps1bar*vgamma' : 0))/(1-vgamma*vbetabar);	// reduced form error in Y2* equation in period 0 when shock = dImpulse
		decl u20 = (m_cY1 ? meps1bar*vgamma' : 0)/(1-vgamma*vbetabar);				// reduced form error in Y2* equation in period 0 when shock = 0
		for(decl t = 0; t < T; ++t){
			decl mu1g2t;
			if(t){
				id1 = id2;
...
**/
{
	...
	decl vgamma;
	if(m_cY1)
		vgamma = this.GetGamma(vbetatil,mOmega);
	else
		vgamma = 0;
