...
main()
{
	...
	decl fusebootIRF = FALSE;                   // if TRUE, do bootstrapped IRFs
	decl fusestoredboot = TRUE;					// if you've already stored the bootstrap replications, set to TRUE to save time. If FALSE, the program will take more than one day to complete on 15 processors
...
//         Perform bootstrap and store bootstrap replications, for computing CIs later
	       ...
	       if(!fusestoredboot){			// if you haven't done it, compute bootstrap replications -- can take more than a day on multiple processors
//		     model.SetStoreBoot(TRUE);
//		     model.SetStoreBootFile(sbootfile);
           ...
           decl fi = fopen(sbootfile, "V");
	       decl modelnew = new CKSVAR();
	       fscan(fi, "%v", &modelnew);
	       mBootPar = modelnew.GetBootstrapData(FALSE)[0][][1:];
	       delete modelnew;
		   fclose(fi);
//	       decl mBootPar = loadmat(sbootfile)[][1:];			// extract bootstrapped estimates of the parameters	(first entry is loglik)
	       vp = mBootPar[0][]';							// first row contains point estimates using SIS
	       model.SetPar(vp);									// redundant step if festimateFAPF = FALSE
	     }
	     else{
	           vp = model.GetPar();
	     }    
